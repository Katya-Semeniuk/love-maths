// Waiting for the DOM finishing loading before running the game
...
/**
 * Checks the answer agains the first element in the
 *returned calculateCorrectAnswer array
 */
function checkAnswer() {
  let userAnswer = parseInt(document.getElementById("answer-box").value);
  let calculatedAnswer = calculateCorrectAnswer(); //return array
...
function calculateCorrectAnswer() {
  ...
  if (operator === "+") {
    ...
    let result = Math.floor(operand1 / operand2);
    console.log(result);
    return [result, "division"];
  } else {
    alert(`Unimplementing operator ${operator}`);
    throw `Unimplementing operator ${operator}. Aborting!`;
  }
}
